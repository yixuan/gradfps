...
# ord = order(diag(Smat), decreasing = TRUE)
# x0diag = numeric(p)
# x0diag[ord[1:d]] = 1
# x0 = diag(x0diag)
# view_matrix(x0[1:200, 1:200])
...
view_matrix(res$projection)



# FPS
res_fps = fps:::fps_benchmark(
    Smat, d, lambda, x0, Pi, rho = -1, maxiter = 100, tolerance = 1e-3, verbose = 0
)
...
res_subgrad = gradfps_subgrad_benchmark(
    Smat, Pi, d, lambda, x0, lr = 0.05, maxiter = 30,
...
res_grad = gradfps_prox_benchmark(
    Smat, Pi, d, lambda, x0w, lr = 0.05, maxiter = 100,
